...
def conv_block(ip1, ip2, ip3, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1E-4):
    ...
    x3 = BatchNormalization(mode=0, axis=concat_axis, gamma_regularizer=l2(weight_decay),
                           beta_regularizer=l2(weight_decay))(ip3)
    ac=Activation('relu')
    x1 = ac(x1)
    x2 = ac(x2)
    x3 = ac(x3)
    ...
    if bottleneck:
        ...
        x3 = BatchNormalization(mode=0, axis=concat_axis, gamma_regularizer=l2(weight_decay),
                           beta_regularizer=l2(weight_decay))(x3)
        ac=Activation('relu')                           
        x1 = ac(x1)
        x2 = ac(x2)
        x3 = ac(x3)
...
def transition_down_block(ip1, ip2, ip3, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4):
...
def transition_up_block(ip1, ip2, ip3, nb_filters, type='subpixel', output_shape=None, weight_decay=1E-4):
...
def dense_block(x1, x2, x3, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1E-4):
    ...
    for i in range(nb_layers):
        x1,x2,x3 = conv_block(x1,x2,x3,growth_rate, bottleneck, dropout_rate, weight_decay)
        feature_list1.append(x1)
        feature_list2.append(x2)
        feature_list3.append(x3)
        x1 = merge(feature_list1, mode='concat', concat_axis=concat_axis)
        x2 = merge(feature_list2, mode='concat', concat_axis=concat_axis)
        x3 = merge(feature_list3, mode='concat', concat_axis=concat_axis)
        nb_filter += growth_rate

    return x1,x2,x3,nb_filter


def create_fc_dense_net(nb_classes, img_dim, hidden1_input_dim, hidden2_input_dim, hidden3_input_dim, nb_dense_block=5, growth_rate=12, nb_filter=16, nb_layers=4, upsampling_conv=128,
    ...
    hidden3_input=Input(shape=hidden3_input_dim)
    hidden3_list=[]
    hidden3_list.append(hidden3_input)
    x = Convolution2D(7, 1, 1, init="he_uniform", subsample=(2,2),
        border_mode="same", bias=False, W_regularizer=l2(weight_decay))(hidden3_input)
    #x = BatchNormalization(mode=0, axis=-1, gamma_regularizer=l2(weight_decay),beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2))(x)
    hidden3_list.append(None)
    hidden3_list.append(x)

    x = Convolution2D(7, 1, 1, init="he_uniform", 
        border_mode="same", bias=False, W_regularizer=l2(weight_decay))(x)
    #x = BatchNormalization(mode=0, axis=-1, gamma_regularizer=l2(weight_decay),beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2))(x)
    ...
    model_input=merge([img_input, hidden1_list[0], hidden2_list[0], hidden3_list[0]],mode='concat')
    ...
    x_t3 = conv_init(model_input)

    skip_connection_t1 = x_t1
    skip_connection_t2 = x_t2
    skip_connection_t3 = x_t3
    ...
    for block_idx in range(nb_dense_block):
        x_t1, x_t2, x_t3, nb_filter = dense_block(x_t1, x_t2, x_t3, nb_layers[block_idx], nb_filter, growth_rate, bottleneck=bottleneck,
        ...
        x_t3 = merge([x_t3, skip_connection_t3], mode='concat', concat_axis=concat_axis)
        skip_list_t3.append(x_t3)

        if hidden1_list[block_idx] is not None: 
            x_t1 = merge([x_t1, hidden1_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden1_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden1_list[block_idx]], mode='concat', concat_axis=concat_axis)
        if hidden2_list[block_idx] is not None: 
            x_t1 = merge([x_t1, hidden2_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden2_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden2_list[block_idx]], mode='concat', concat_axis=concat_axis)
        if hidden3_list[block_idx] is not None: 
            x_t1 = merge([x_t1, hidden3_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden3_list[block_idx]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden3_list[block_idx]], mode='concat', concat_axis=concat_axis)


        # add transition_block
        x_t1, x_t2, x_t3 = transition_down_block(x_t1, x_t2, x_t3, nb_filter, compression=compression, dropout_rate=dropout_rate,
    ...
    x_t1, x_t2, x_t3, nb_filter = dense_block(x_t1, x_t2, x_t3, final_nb_layer, nb_filter, growth_rate, bottleneck=bottleneck,
                               dropout_rate=dropout_rate, weight_decay=weight_decay)

    # Add dense blocks and transition up block
    for block_idx in range(nb_dense_block):
        if hidden1_list[-(block_idx+1)] is not None: 
            x_t1 = merge([x_t1, hidden1_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden1_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden1_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
        if hidden2_list[-(block_idx+1)] is not None: 
            x_t1 = merge([x_t1, hidden2_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden2_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden2_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
        if hidden3_list[-(block_idx+1)] is not None: 
            x_t1 = merge([x_t1, hidden3_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t2 = merge([x_t2, hidden3_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)
            x_t3 = merge([x_t3, hidden3_list[-(block_idx+1)]], mode='concat', concat_axis=concat_axis)

        x_t1, x_t2, x_t3 = transition_up_block(x_t1, x_t2, x_t3, nb_filters=upsampling_conv, type=upscaling_type)
        ...
        x_t3 = merge([x_t3, skip_list_t3.pop()], mode='concat', concat_axis=concat_axis)

        x_t1, x_t2, x_t3, nb_filter = dense_block(x_t1,x_t2, x_t3, nb_layers[-block_idx], nb_filter, growth_rate, bottleneck=bottleneck,
                                   dropout_rate=dropout_rate, weight_decay=weight_decay)

    x_t1 = merge([x_t1, hidden1_input,hidden2_input, hidden3_input], mode='concat', concat_axis=concat_axis)
    x_t2 = merge([x_t2, hidden1_input,hidden2_input, hidden3_input], mode='concat', concat_axis=concat_axis)
    x_t3 = merge([x_t3, hidden1_input,hidden2_input, hidden3_input], mode='concat', concat_axis=concat_axis)
    ...
    if nb_classes>0:
        ...
        x_t3 = Reshape((row * col, nb_classes))(x_t3)
        x_t1 = Activation('softmax')(x_t1)
        x_t2 = Activation('softmax')(x_t2)
        x_t3 = Activation('softmax')(x_t3)

    densenet_t1 = Model(input=[img_input, hidden1_input, hidden2_input, hidden3_input], output=x_t1, name="create_dense_net_t1")
    densenet_t2 = Model(input=[img_input, hidden1_input, hidden2_input, hidden3_input], output=x_t2, name="create_dense_net_t2")
    densenet_t3 = Model(input=[img_input, hidden1_input, hidden2_input, hidden3_input], output=x_t3, name="create_dense_net_t3")

    # Compute depth
    nb_conv_layers = len([layer.name for layer in densenet_t3.layers
    ...
    return densenet_t1, densenet_t2, densenet_t3
